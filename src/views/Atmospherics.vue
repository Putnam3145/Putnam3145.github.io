...
">
        ...
        <p>But wouldn’t this cause lag, due to both increasing the total workload by never pausing turfs and never pausing to let the game run? Well, as mentioned earlier, this is highly parallelizable. Each individual gas mixture can, in theory, be done completely independently from the others, with only some read-sharing involved. In practice, I use a <b-link href="https://github.com/xacrimon/dashmap">sharded hash map implementation called DashMap</b-link> and <b-link href="https://crates.io/crates/rayon">Rayon</b-link> to run each individual shard of the DashMap in parallel, which leads to a pretty much linear speedup with number of cores. On top of that, the entire process is started on another core, meaning that the game experiences no interruption as it runs—in fact, the game starts the thread, does a bunch of processing relating to atmos that isn’t on tiles while the thread is running, then waits for the thread to finish if it hasn’t yet before doing turf operations. In practice, it usually finishes far before it gets to that point, meaning that, as far as the player knows, this entire process causes zero performance impact.</p>

        <p>There's other reasons to do it this way. Doing everything at once requires less bookkeeping, and is thus simpler, using less memory; the archive no longer being required means there’s further less memory being allocated. Since SS13 is still 32-bit, this provides far more breathing room.</p>
