...
">
    First, we calculate the difference in temperatures. Using this, we then calculate the heat difference via some basic dimensional analysis (you don't have to think about it too hard, in other words) and exchange heat that way. We multiply the heat exchange by the conduction coefficient, which in this case is 0.4.
        ...
        <p>I decided this should be rectified. I switched all of the gas mixture code to more modern C++ constructions, making gas mixtures stored in a vector and using the STL instead of <code>memcopy</code> and such. Most importantly, I switched the raw pointer saving with a simple vector that saves deallocated gas mixture locations<citation id="8">This turns out to essentially be <b-link href="https://en.wikipedia.org/wiki/Region-based_memory_management">region-based memory management.</b-link></citation> This made the whole thing something like 3-4 times as fast, probably primarily due to cache coherence<citation id="9">So easy to blame the cache coherence.</citation>, though auto-vectorization did help. At this point, everything seemed good... until I couldn’t update atmos anymore without causing lavaland, the planetary mining zone in TGstation servers, to explode, usually instantly, with baffling floating point errors.</p> 
...
">
        ...
        <p>This is all an open source project. I'm hoping that I won't be the only person on the planet who understands it well enough to work on it <i>and</i> cares enough to actually do it. By writing this, I want either people who care to learn how to work on it or people who can work on it to start caring. I don't mind working on these things, I enjoy it greatly<citation id="citation needed">Actually, no, a <b-link>private jet</b-link> would be unethical, huh?</citation>, but the <b-link href="https://en.wikipedia.org/wiki/Bus_factor">bus factor</b-link> is a terrifying thing, and I hope to reduce it. Please consider <b-link href="https://github.com/Putnam3145/auxmos">contributing to Auxmos</b-link>, even if it's providing insights I might not have thoguht of (this happens more often than you'd think).</p>
    </b-container>
</template>
